...
async function viewSinglePurchaseOrderCTR(req, res) {
  const data = req.body;
  ...
  try {
    ...
    const items = await PurchaseOrder.findOne(query)
      .sort({ createdAt: -1 })
      .populate({
        path: ["createdBy", "updatedBy", "itemDetails.code", "supplier"],
        select:
          "name code SKU phone email TIN taxInfo officeAddress paymentInfo",
      })
      .lean();
    if (!items) {
...
module.exports = viewSinglePurchaseOrderCTR;
